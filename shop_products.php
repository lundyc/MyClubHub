...
if ($isAdmin && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!csrf_check()) {
        header('Location: /shop_products.php?m=' . rawurlencode('Session expired — try again.'));
        exit;
    }
    ...
    try {
        ...
        header('Location: /shop_products.php' . ($msg !== '' ? '?m=' . rawurlencode($msg) : ''));
        exit;
    } catch (Throwable $e) {
        header('Location: /shop_products.php?e=' . rawurlencode($e->getMessage()));
        exit;
    }
}
...
<div class="shop-admin-page">
    <nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/shop_overview.php">Shop</a> <i class="fa-solid fa-chevron-right" aria-hidden="true"></i> <span aria-current="page">Products</span></nav>
    ...
    <section class="card hub-panel table-responsive">
        <table class="table align-middle mb-0">
            ...
            <tbody>
            <?php if (!$products): ?>
                <tr><td colspan="8" class="text-muted text-center py-4">No products yet. <a href="/shop_product.php">Add one</a>.</td></tr>
            <?php endif; ?>
            <?php foreach ($products as $p): ?>
                ...
                <tr>
                    ...
                    <td class="fw-bold"><a class="text-decoration-none" href="/shop_product.php?id=<?= (int) $p['id'] ?>"><?= h((string) $p['name']) ?></a>
                    ...
                    <td class="text-end text-nowrap">
                        <a class="btn btn-sm btn-outline-secondary" href="/shop_product.php?id=<?= (int) $p['id'] ?>">Edit</a>
