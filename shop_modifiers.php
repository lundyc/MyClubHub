...
if ($isAdmin && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!csrf_check()) {
        header('Location: /shop_modifiers.php?e=' . rawurlencode('Session expired — try again.'));
        exit;
    }
    ...
    try {
        if ($action === 'save_group') {
            ...
            header('Location: /shop_modifiers.php?group=' . $savedId . '&m=' . rawurlencode('Modifier saved.'));
            exit;
        }
        if ($action === 'delete_group') {
            shop_delete_modifier_group($pdo, (int) ($_POST['id'] ?? 0));
            auditLog($pdo, 'shop_modifier_group_deleted', 'Modifier group #' . (int) ($_POST['id'] ?? 0));
            header('Location: /shop_modifiers.php?m=' . rawurlencode('Modifier deleted.'));
            exit;
        }
        ...
        header('Location: /shop_modifiers.php');
        exit;
    } catch (Throwable $e) {
...
<div class="shop-admin-page">
    <nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/shop_overview.php">Shop</a> <i class="fa-solid fa-chevron-right" aria-hidden="true"></i> <span aria-current="page">Modifiers</span></nav>
        ...
        <div class="col-lg-4">
            <section class="card hub-panel">
                <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                    ...
                    <a class="btn btn-sm btn-dark" href="/shop_modifiers.php?new=1">New</a>
                </div>
                <div class="list-group list-group-flush">
                    <?php if (!$groups): ?><div class="list-group-item text-muted">None yet.</div><?php endif; ?>
                    <?php foreach ($groups as $g): ?>
                        <a class="list-group-item list-group-item-action d-flex justify-content-between align-items-center <?= (int) $g['id'] === $selectedId ? 'active' : '' ?>" href="/shop_modifiers.php?group=<?= (int) $g['id'] ?>">
                ...
                <section class="card hub-panel p-3">
                            ...
                            <?php foreach ($options as $o): ?>
                                <tr>
                                    ...
                                    <td class="text-end text-nowrap">
                                        <a class="btn btn-sm btn-outline-secondary" href="/shop_modifiers.php?group=<?= (int) $group['id'] ?>&option=<?= (int) $o['id'] ?>">Edit</a>
                    ...
                    <form method="post" class="row g-2">
                        ...
                        <div class="col-12 d-flex gap-2">
                            ...
                            <?php if ($editOption): ?><a class="btn btn-outline-secondary" href="/shop_modifiers.php?group=<?= (int) $group['id'] ?>">Cancel</a><?php endif; ?>
                        </div>
                    </form>
                </section>
            <?php elseif (!$newGroup): ?>
