...
if ($isAdmin && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!csrf_check()) {
        header('Location: /shop_products.php?e=' . rawurlencode('Session expired — try again.'));
        exit;
    }
    ...
    try {
        if ($action === 'save') {
            ...
            header('Location: /shop_product.php?id=' . $savedId . '&m=' . rawurlencode('Product saved.'));
            exit;
        }
        if ($action === 'add_image' && $id) {
            ...
            header('Location: /shop_product.php?id=' . $id . '&m=' . rawurlencode($uploaded !== null ? 'Image added.' : 'That file was not a valid image.'));
            exit;
        }
        if ($action === 'delete_image' && $id) {
            ...
            header('Location: /shop_product.php?id=' . $id . '&m=' . rawurlencode('Image removed.'));
            exit;
        }
        if ($action === 'delete' && $id) {
            shop_delete_product($pdo, $id);
            auditLog($pdo, 'shop_product_deleted', 'Shop product #' . $id);
            header('Location: /shop_products.php?m=' . rawurlencode('Product deleted.'));
            exit;
        }
        header('Location: /shop_products.php');
        exit;
    } catch (HubFieldValidationException $e) {
        header('Location: /shop_product.php?' . ($id ? 'id=' . $id . '&' : '') . 'e=' . rawurlencode($e->getMessage()));
        exit;
    } catch (Throwable $e) {
        header('Location: /shop_product.php?' . ($id ? 'id=' . $id . '&' : '') . 'e=' . rawurlencode($e->getMessage()));
        exit;
...
<div class="shop-admin-page">
    <nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/shop_overview.php">Shop</a> <i class="fa-solid fa-chevron-right" aria-hidden="true"></i> <a href="/shop_products.php">Products</a> <i class="fa-solid fa-chevron-right" aria-hidden="true"></i> <span aria-current="page"><?= $product ? h((string) $product['name']) : 'New' ?></span></nav>
    ...
    <?php if (!$categories): ?>
        <div class="alert alert-warning">Create a <a href="/shop_categories.php">category</a> first.</div>
    <?php else: ?>
        ...
        <div class="row g-3">
            <div class="col-lg-8">
                ...
                <section class="card hub-panel p-3">
                    ...
                    <?php if (!$allGroups): ?>
                        <p class="text-muted mb-0">No modifier groups yet. <a href="/shop_modifiers.php">Create one</a> (e.g. "Kit Size") then come back.</p>
                    <?php else: ?>
