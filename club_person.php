...
{
    header('Location: /club_person.php?id=' . $personId . '&status=' . rawurlencode($status));
    exit;
}
    ...
    if (!hub_auth_verify_csrf_token((string) ($_POST['csrf_token'] ?? ''))) {
        ...
        try {
            ...
            if ($action === 'archive_person') {
                ...
                header('Location: /club_people.php?status=all&status_message=archived');
                exit;
            }
...
<nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/club_people.php">People &amp; Users</a><i class="fa-solid fa-chevron-right" aria-hidden="true"></i><span aria-current="page"><?= $isNew ? 'Add person' : h((string) $data['display_name']) ?></span></nav>
    ...
    <div class="col-xl-7">
        <form method="post" class="card shadow-sm border-0">
            ...
            <div class="card-footer d-flex justify-content-between gap-2">
                <a class="btn btn-outline-secondary" href="/club_people.php">Cancel</a>
                        ...
                        <?php if ($seasonTicketHistory): ?>
                            <div class="people-history-list">
                                <?php foreach ($seasonTicketHistory as $order): ?>
                                    <a class="people-history-item" href="/season_ticket_orders.php?id=<?= (int) $order['order_id'] ?>">
                        ...
                        <?php if ($matchTicketHistory): ?>
                            <div class="people-history-list">
                                <?php foreach ($matchTicketHistory as $order): ?>
                                    <a class="people-history-item" href="/ticket_orders.php?id=<?= (int) $order['id'] ?>">
...
<script src="/assets/js/club-person.js?v=<?= (int) (@filemtime(__DIR__ . '/assets/js/club-person.js') ?: time()) ?>" defer></script>
