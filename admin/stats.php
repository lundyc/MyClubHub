...
$tab = is_string($_GET['tab'] ?? null) && in_array($_GET['tab'], ['team', 'players', 'matches', 'compare'], true) ? $_GET['tab'] : 'team';
...
$url = static function (array $changes) use ($seasonId, $tab, $competition, $venue, $compareSeasonIds, $selectedCompetitions, $compareMetrics): string {
...
<div class="container-fluid py-3">
    <nav class="nav nav-tabs mb-4" aria-label="Statistics sections">
        <?php foreach (['team' => 'Team stats', 'players' => 'Player Stats', 'matches' => 'Match Stats', 'compare' => 'Compare seasons'] as $key => $label): ?>
    ...
    <?php if ($tab === 'compare'): ?>
        ...
        <script>
        window.STATS_COMPARE_FILTERS = <?= json_encode(['seasons' => $compareSeasonIds, 'competitions' => $selectedCompetitions, 'venue' => $venue], JSON_UNESCAPED_SLASHES) ?>;
        (function () {
        ...
        (function () {
            ...
            // detail behind that number (which games, and when) for the currently compared
            // seasons/competitions/venue.
            ...
            var modal = null;
            var filters = window.STATS_COMPARE_FILTERS || { seasons: [], competitions: [], venue: '' };
                ...
                btn.addEventListener('click', function () {
                    ...
                    fetch('/admin/stats_player_detail.php?' + params.toString())
                        ...
                        .then(function (data) {
                            ...
                            var html = '<p class="text-muted small">' + data.total + ' total across ' + data.rows.length + ' match' + (data.rows.length === 1 ? '' : 'es') + '.</p>';
                            html += '<div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead><tr>'
                            ...
                            data.rows.forEach(function (row) {
                                ...
                                html += '<tr><td class="text-nowrap">' + esc(dateStr) + '</td><th scope="row">' + esc(row.opponent) + '</th><td>' + esc(row.venue) + '</td><td>' + esc(row.competition) + '</td><td class="text-nowrap">' + esc(row.score || '—') + '</td><td>' + esc(row.detail) + '</td></tr>';
                            });
