      ...
      <section class="player-reference-section" data-reference-section="squad">
        <header class="player-reference-section__header">
          ...
          <div class="player-reference-section__header-actions">
            ...
            <a href="/player_reference_pdf.php" class="btn btn-brand btn-sm"><i class="fa-solid fa-file-pdf me-1" aria-hidden="true"></i>Export PDF</a>
          </div>
        </header>
...
(() => {
  ...
  async function uploadPlayerPhoto(card, file) {
    ...
    try {
      const response = await fetch('/player_reference_avatar_upload.php', {
        method: 'POST',
  ...
  async function acceptPlayerPhoto(card, button) {
    ...
    try {
      ...
      const response = await fetch('/player_photo_review_save.php', {
        method: 'POST',
        credentials: 'same-origin',
  ...
  async function setWebsiteStatus(card, toggle, checkbox, uploaded) {
    ...
    try {
      ...
      const response = await fetch('/player_website_photo_save.php', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
        body
  ...
  document.querySelectorAll('[data-player-card]').forEach((card) => {
    ...
    select.addEventListener('change', async () => {
      ...
      try {
        ...
        const response = await fetch('/player_position_save.php', {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
          body
