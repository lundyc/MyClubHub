...
echo '<script src="/assets/js/match_media.js?v=' . h($matchMediaJsVersion) . '" defer></script>';
?>
<link rel="stylesheet" href="/assets/css/match_media.css">
<link rel="stylesheet" href="/assets/css/player_edit.css">

<nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/matches.php">Fixtures</a><i class="fa-solid fa-chevron-right" aria-hidden="true"></i><a href="/match.php?id=<?= (int) $fixtureId ?>&amp;season_id=<?= (int) $seasonId ?>"><?= h($opponent) ?></a><i class="fa-solid fa-chevron-right" aria-hidden="true"></i><span aria-current="page">Media</span></nav>
...
<div>
    ...
    <div class="card shadow-sm hub-form-card mb-4">
        <div class="card-body">
            ...
            <p class="player-edit-help">Everyone in each photo is tagged automatically by comparing faces against the squad's profile pictures and everyone's reference photos on the <a href="/people.php">People</a> page. Check the results below — tags can be added or removed at any time.</p>
            <form method="post" enctype="multipart/form-data" action="/match_photo_upload.php" id="matchPhotoUploadForm">
                <?= csrf_field() ?>
                    ...
                    <?php foreach ($photos as $photo): ?>
                        <div class="match-media-item" data-photo-id="<?= (int) $photo['id'] ?>">
                            <div class="match-media-item__image">
                                ...
                                <form method="post" action="/match_photo_delete.php" class="match-media-item__delete">
                                    <?= csrf_field() ?>
