...
$totals = $pdo->query("SELECT
    ...
    FROM shop_orders")->fetch(PDO::FETCH_ASSOC) ?: [];

// Units sold (paid) per product + option.
$breakdown = $pdo->query("SELECT i.product_name, i.options_label, SUM(i.quantity) AS qty, SUM(i.line_total) AS revenue
    FROM shop_order_items i JOIN shop_orders o ON o.id = i.order_id
    WHERE o.status IN ('paid','collected','refunded')
    GROUP BY i.product_name, i.options_label
    ORDER BY i.product_name, qty DESC")->fetchAll(PDO::FETCH_ASSOC);

$recent = shop_order_list($pdo, ['limit' => 12]);
...
<div class="shop-admin-page">
    ...
    <?php if (!$stripeReady): ?>
        <div class="alert alert-warning">Stripe is not configured yet, so customers can't pay. Add a secret key under
            <a href="/settings.php?tab=payments">Settings &rsaquo; Payments</a>.</div>
    <?php endif; ?>
    <?php if (($settings['shop_enabled'] ?? '1') !== '1'): ?>
        <div class="alert alert-secondary">The storefront is currently switched <strong>off</strong> in
            <a href="/shop_settings.php">Shop settings</a>.</div>
    <?php endif; ?>

    <div class="row g-3 mb-3">
        <div class="col-6 col-lg-3"><div class="card hub-panel p-3 h-100"><div class="text-muted small text-uppercase fw-bold">Revenue (paid)</div><div class="h3 mb-0"><?= gbp((float) ($totals['revenue'] ?? 0)) ?></div><?php if ((float) ($totals['refunded'] ?? 0) > 0): ?><div class="small text-muted">less <?= gbp((float) $totals['refunded']) ?> refunded</div><?php endif; ?></div></div>
    ...
    <div class="row g-3">
        <div class="col-lg-7">
            <section class="card hub-panel">
                <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                    ...
                    <a class="btn btn-sm btn-outline-secondary" href="/shop_orders.php">All orders</a>
                </div>
                <div class="table-responsive">
                    <table class="table align-middle mb-0">
                        ...
                        <?php foreach ($recent as $o): ?>
                            <tr>
                                <td><a href="/shop_order.php?id=<?= (int) $o['id'] ?>" class="fw-bold text-decoration-none"><?= h((string) $o['order_ref']) ?></a></td>
        ...
        <div class="col-lg-5">
            ...
            <div class="d-grid gap-2 mt-3">
                <a class="btn btn-outline-secondary" href="/shop_products.php"><i class="fa-solid fa-shirt me-1"></i>Manage products</a>
                <a class="btn btn-outline-secondary" href="/shop_modifiers.php"><i class="fa-solid fa-sliders me-1"></i>Manage modifiers &amp; sizes</a>
                <a class="btn btn-outline-secondary" href="/shop_settings.php"><i class="fa-solid fa-gear me-1"></i>Shop settings &amp; pre-order window</a>
            </div>
        </div>
    </div>
</div>
