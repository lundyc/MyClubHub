...
<nav class="hub-breadcrumb" aria-label="Breadcrumb"><a href="/players.php">Players</a><i class="fa-solid fa-chevron-right" aria-hidden="true"></i><span aria-current="page"><?= h((string)($player['name'] ?? 'Player')) ?></span></nav>
  ...
  <section class="card player-profile-details hub-panel">
    <div class="card-body">
      <div class="player-profile-identity">
        ...
        <a href="/player_edit.php?id=<?= $id ?>" class="btn btn-outline-primary btn-sm ms-md-auto"><i class="fa-solid fa-pen me-1" aria-hidden="true"></i>Edit player</a>
      </div>
  ...
  <section class="card player-profile-actionshots hub-panel">
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
        ...
        <a href="/player_edit.php?id=<?= $id ?>" class="btn btn-outline-primary btn-sm"><i class="fa-solid fa-cloud-arrow-up me-1" aria-hidden="true"></i>Manage photos</a>
      </div>
      <?php if ($actionShots): ?>
                    ...
                    <?php foreach ($paymentsBySponsor[$s['sponsor_id']] as $pay): ?>
                      <tr id="payment-row-<?= (int) $pay['id'] ?>">
                        ...
                        <td data-label="Actions" class="player-profile-payment-actions text-end">
                          <a href="/payment_edit.php?id=<?= (int) $pay['id'] ?>" class="btn btn-sm btn-outline-primary">Edit</a>
              ...
              <?php if ($outstandingSlots): ?>
                <div class="player-profile-stripe-links text-end mt-2">
                  <?php foreach ($outstandingSlots as $slot): ?>
                    <a href="/sponsorship_agreement.php?id=<?= (int) $slot['agreement_id'] ?>#stripePaymentCard" class="btn btn-sm btn-outline-primary" title="Send Stripe payment link">
